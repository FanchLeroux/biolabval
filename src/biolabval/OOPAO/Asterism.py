...
from biolabval.OOPAO.tools.displayTools import makeSquareAxes


class Asterism:
    def __init__(self, list_src: list):
        """
        ************************** REQUIRED PARAMETERS **************************
    ...
    def display_asterism(self):
        ...
        for i_src in range(self.n_source):
            if self.src[i_src].type == "NGS":
                ...
                size_marker = 10
            plt.plot(
                self.src[i_src].coordinates[0]
                * np.cos(np.deg2rad(self.src[i_src].coordinates[1])),
                self.src[i_src].coordinates[0]
                * np.sin(np.deg2rad(self.src[i_src].coordinates[1])),
                marker,
                markersize=size_marker,
                color=cm(1.0 * i_src / self.n_source),
                markeredgecolor="k",
    ...
    def properties(self) -> dict:
        self.prop = dict()
        self.prop["parameters"] = (
            f"{'Source':^8s}|{'Wavelength':^12s}|{'Zenith':^8s}|{'Azimuth':^9s}|{'Altitude':^10s}|{'Magnitude':^11s}|{'Flux':^11s}|"
        )
        self.prop["units"] = (
            f"{'':^8s}|{'[m]':^12s}|{'[arcsec]':^8s}|{'[°]':^9s}|{'[m]':^10s}|{'':^11s}|{'[ph/m²/s]':^11s}|"
        )
        for i in range(self.n_source):
            if i % 2 == 0:
                self.prop["layer_%02d" % i] = (
                    f"\033[00m{'%3d-%s'%(i+1,self.src[i].type):^8s}|{self.src[i].wavelength:^12.1e}|{self.src[i].coordinates[0]:^8.2f}|{self.src[i].coordinates[1]:^9.2f}|{self.src[i].altitude:^10.2f}|{self.src[i]._magnitude:^11.2f}|{self.src[i]._nPhoton:^11.1e}|"
                )
            else:
                self.prop["layer_%02d" % i] = (
                    f"\033[47m{'%3d-%s'%(i+1,self.src[i].type):^8s}|{self.src[i].wavelength:^12.1e}|{self.src[i].coordinates[0]:^8.2f}|{self.src[i].coordinates[1]:^9.2f}|{self.src[i].altitude:^10.2f}|{self.src[i]._magnitude:^11.2f}|{self.src[i]._nPhoton:^11.1e}|"
                )
        return self.prop
