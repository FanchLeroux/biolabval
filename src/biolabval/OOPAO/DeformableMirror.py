...
class DeformableMirror:
    def __init__(
        ...
        ************************** EXEMPLE **************************
        ...
        self.rad2arcsec = (180.0 / np.pi) * 3600
        ...
        if M4_param is not None:
            if M4_param["isM4"]:
                from .M4_model.make_M4_influenceFunctions import (
                    makeM4influenceFunctions,
                )

                print_(
                    "Building the set of influence functions of M4...",
                    print_dm_properties,
                )
                ...
                try:
                    coordinates_M4 = makeM4influenceFunctions(
                        pup=pup,
                        filename=filename,
                        misReg=misReg,
                        dm=self,
                ...
                if M4_param["validActCriteria"] != 0:
                    IF_STD = np.std(
                        np.squeeze(self.modes[telescope.pupilLogical, :]), axis=0
                    )
                    ACTXPC = np.where(
                        IF_STD >= np.mean(IF_STD) * M4_param["validActCriteria"]
                    )
                ...
                b = time.time()
                print_(
                    "Done! M4 influence functions computed in " + str(b - a) + " s!",
                    print_dm_properties,
                )
            else:
                self.isM4 = False
        ...
        if altitude is None:
            ...
            if telescope.src.tag == "asterism":
                self.oversampling_factor = np.max(
                    (
                        np.asarray(self.telescope.src.coordinates)[:, 0]
        ...
        if coordinates is None:
            ...
            if self.actuator_selection is None:
                r_in = (
                    telescope.centralObstruction * telescope.initial_D / 2
                    - 0.5 * self.pitch
                )
            ...
            if np.isscalar(self.actuator_selection):
                ...
                if len(self.actuator_selection) != 2:
                    raise OopaoError(
                        "actuator_selection must be either a scalar or a vector of length 2 to specify the inner and outer radius."
                    )
        ...
        # anamorphosis
        xIF3, yIF3 = self.anamorphosis_coordinates(
            xIF0,
            yIF0,
        ...
        # rotation
        xIF4, yIF4 = self.rotate_coordinates(
        ...
        if self.isM4 is False:
            ...
            if modes is None:
                ...
                self.modes = np.squeeze(
                    np.moveaxis(np.asarray(joblib_construction()), 0, -1)
                )
                if np.isscalar(self.actuator_selection):
                    print_(
                        "Filtering valid actuators based on pupil influence functions std...",
                        print_dm_properties,
                    )
                    # Calculate std only within the illuminated pupil
                    IF_STD = np.std(
                        np.squeeze(
                            self.modes[np.where(telescope.pupil.flatten() == 1), :]
                        ),
                        axis=0,
                    )
                    self.validAct = np.where(
                        IF_STD > self.actuator_selection * np.max(IF_STD)
                    )[0]
    ...
    def set_pupil_footprint(self):
        if len(self.src_list) == 1:
            [x_z, y_z] = pol2cart(
                ...
                * xp.tan(self.src_list[0].coordinates[0] / self.rad2arcsec)
                * self.altitude_layer.resolution
                / self.altitude_layer.D,
                xp.deg2rad(self.src_list[0].coordinates[1]),
            )
            ...
            self.altitude_layer.pupil_footprint = xp.zeros(
                [self.altitude_layer.resolution, self.altitude_layer.resolution],
                dtype=self.precision(),
            )
            self.altitude_layer.pupil_footprint[
                center_x
                - self.telescope.resolution // 2 : center_x
                + self.telescope.resolution // 2,
                center_y
                - self.telescope.resolution // 2 : center_y
                + self.telescope.resolution // 2,
            ] = 1
        else:
            self.altitude_layer.pupil_footprint = []
            ...
            for src in self.src_list:
                [x_z, y_z] = pol2cart(
                    self.altitude_layer.altitude
                    * xp.tan(src.coordinates[0] / self.rad2arcsec)
                    * self.altitude_layer.resolution
                    / self.altitude_layer.D,
                    xp.deg2rad(src.coordinates[1]),
                )
    ...
    def buildLayer(self, telescope, altitude):
        ...
        if telescope.src.tag == "source":
            [x_z, y_z] = pol2cart(
                layer.altitude
                * xp.tan(telescope.src.coordinates[0] / self.rad2arcsec)
                * layer.resolution
                / layer.D,
                xp.deg2rad(telescope.src.coordinates[1]),
            )
            ...
            layer.pupil_footprint[
                center_x
                - telescope.resolution // 2 : center_x
                + telescope.resolution // 2,
                center_y
                - telescope.resolution // 2 : center_y
                + telescope.resolution // 2,
            ] = 1
        else:
            ...
            for i in range(telescope.src.n_source):
                [x_z, y_z] = pol2cart(
                    layer.altitude
                    * xp.tan(telescope.src.coordinates[i][0] / self.rad2arcsec)
                    * layer.resolution
                    / layer.D,
                    xp.deg2rad(telescope.src.coordinates[i][1]),
                )
    ...
    def get_OPD_altitude(self, src):
        ...
        if np.ndim(self.OPD) == 2:
            if src.inAsterism:
                OPD = np.reshape(
                    self.OPD[
                        np.where(
                            self.altitude_layer.pupil_footprint[self.src.ast_idx] == 1
                        )
                    ],
                ...
                OPD = np.reshape(
                    self.OPD[np.where(self.altitude_layer.pupil_footprint == 1)],
                    [self.telescope.resolution, self.telescope.resolution],
                )
        else:
            OPD = np.reshape(
                self.OPD[
                    self.altitude_layer.center_x[self.src.ast_idx]
                    - self.telescope.resolution
                    // 2 : self.altitude_layer.center_x[self.src.ast_idx]
                    + self.telescope.resolution // 2,
                    self.altitude_layer.center_y[self.src.ast_idx]
                    - self.telescope.resolution
                    // 2 : self.altitude_layer.center_y[self.src.ast_idx]
                    + self.telescope.resolution // 2,
                    :,
                ],
                [
        ...
        if ~np.isinf(src.altitude):
            ...
            OPD = np.asarray(
                np.squeeze(
                    interpolate_cube(
                        cube_in, pixel_size_in, pixel_size_out, resolution_out
                    )
                ).T
            )
    ...
    def anamorphosis_coordinates(self, x, y, angle, mRad, mNorm):
        ...
        mRad += 1
        mNorm += 1
        xOut = x * (mRad * np.cos(angle) ** 2 + mNorm * np.sin(angle) ** 2) + y * (
            mNorm * np.sin(2 * angle) / 2 - mRad * np.sin(2 * angle) / 2
        )
        yOut = y * (mRad * np.sin(angle) ** 2 + mNorm * np.cos(angle) ** 2) + x * (
            mNorm * np.sin(2 * angle) / 2 - mRad * np.sin(2 * angle) / 2
        )
        return xOut, yOut
    ...
    def modesComputation(self, i, j):
        ...
        y0 = j
        cx = (
            (1 + self.misReg.radialScaling)
            * (self.resolution / self.nActAlongDiameter)
            / np.sqrt(2 * np.log(1.0 / self.mechCoupling))
        )
        cy = (
            (1 + self.misReg.tangentialScaling)
            * (self.resolution / self.nActAlongDiameter)
            / np.sqrt(2 * np.log(1.0 / self.mechCoupling))
        )
    ...
    def display_dm(self, fig_index=None, list_src=None, input_opd=None):
        ...
        gs = gridspec.GridSpec(
            1, 1, height_ratios=[1], width_ratios=[1], hspace=0.5, wspace=0.5
        )
        ax = plt.subplot(gs[0, 0])
        if input_opd is None:
            input_opd = np.reshape(
                np.sum(self.modes**5, axis=1), [self.resolution, self.resolution]
            )
        ...
        for i_source in range(len(list_src)):
            col.append(cm(1.0 * i_source / len(list_src)))
            [x_c, y_c] = pol2cart(
                self.telescope.initial_D / 2,
                xp.linspace(0, 2 * xp.pi, 100, endpoint=True),
            )
            ...
            if self.altitude is None:
                ...
                [x_z, y_z] = pol2cart(
                    self.altitude
                    * xp.tan((list_src[i_source].coordinates[0]) / self.rad2arcsec),
                    -xp.deg2rad(list_src[i_source].coordinates[1]),
                )
            center = 0
            [x_c, y_c] = pol2cart(
                self.D / 2, xp.linspace(0, 2 * xp.pi, 100, endpoint=True)
    ...
    def coefs(self, val):
        ...
        if np.isscalar(val):
            if val == 0:
                ...
                try:
                    self.OPD = self.precision(
                        np.reshape(
                            np.matmul(self.modes, self._coefs),
    ...
    def set_coefs_value(self, actuator_index: list, actuator_coefs: list):
        ...
        if len(actuator_index) != len(actuator_coefs):
            raise OopaoError(
                "The list of actuator_index and actuator_coefs must have the same length"
            )
    ...
    def properties(self) -> dict:
        self.prop = dict()
        self.prop["controlled_act"] = (
            f"{'Controlled Actuators':<25s}|{self.nValidAct:^9.0f}"
        )
        self.prop["is_m4"] = f"{'M4':<25s}|{str(self.isM4):^9s}"
        self.prop["pitch"] = f"{'Pitch [m]':<25s}|{self.pitch:^9.2f}"
        self.prop["mechanical_coupling"] = (
            f"{'Mechnical coupling [%]':<25s}|{self.mechCoupling*100:^9.0f}"
        )
        self.prop["delimiter"] = ""
