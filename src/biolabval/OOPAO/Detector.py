...
class Detector:
    def __init__(
        ...
        QE: float = 1.0,
        binning: int = 1,
        psf_sampling: float = 2.0,
        darkCurrent: float = 0.0,
        ...
        self.random_state_photon_noise = np.random.RandomState(
            seed=int(time.time())
        )  # random states to reproduce sequences of noise
    ...
    def rebin(self, arr, new_shape):
        ...
        out = (
            (arr.reshape(shape).mean(-1).mean(1))
            * (arr.shape[0] // new_shape[0])
            * (arr.shape[1] // new_shape[1])
        )
        return out
    ...
    def set_saturation(self, frame):
        ...
        if frame.max() > self.FWC:
            warning("The detector is saturating, %.1f %%" % self.saturation)
    ...
    def digitalization(self, frame):
        if self.FWC is None:
            ...
            if frame.max() > self.FWC:
                warning(
                    "The ADC is saturating (gain applyed %i), %.1f %%"
                    % (self.gain, self.saturation)
                )
            frame = frame / self.FWC * (2**self.bits - 1)
            return np.clip(
                frame,
                a_min=max(frame.min(), self.clip_unsigned),
                a_max=2**self.bits - 1,
            )
    ...
    def set_background_noise(self, frame):
        if hasattr(self, "backgroundFlux") is False or self.backgroundFlux is None:
            raise OopaoError(
                "The background map backgroundFlux is not properly set. A map of shape "
                + str(frame.shape)
                + " is expected"
            )
        else:
    ...
    def set_readout_noise(self, frame):
        noise = (
            np.round(
                self.random_state_readout_noise.randn(frame.shape[0], frame.shape[1])
                * self.readoutNoise
            )
        ).astype(
            int
    ...
    def remove_bakground(self, frame):
        try:
            ...
            raise OopaoError(
                "The shape of the backgroung map does not match the detector frame resolution"
            )
    ...
    def computeSNR(self):
        ...
        self.SNR = self.signal / np.sqrt(
            self.quantification_noise**2
            + self.photon_noise**2
            + self.readoutNoise**2
            + self.dark_shot_noise**2
        )
        print()
    ...
    def displayNoiseError(self):
        ...
        if self.darkCurrent != 0:
            print(
                "{:^25s}|{:^9.4f}".format("Dark shot noise [e-]", self.dark_shot_noise)
            )
        if self.readoutNoise != 0:
    ...
    def backgroundNoise(self, val):
        ...
        if val is True:
            if hasattr(self, "backgroundFlux") is False or self.backgroundFlux is None:
                warning(
                    "The background noise is enabled but no property backgroundFlux is set.\nA map of shape "
    ...
    def properties(self) -> dict:
        ...
        if self.resolution is not None:
            self.prop["resolution"] = (
                f"{'Resolution [px]':<25s}|{int(self.resolution//self.binning):^9d}"
            )
        if self.integrationTime is not None:
            self.prop["exposure"] = (
                f"{'Exposure time [ms]':<25s}|{self.integrationTime*1000:^9.2f}"
            )
        if self.bits is not None:
        ...
        self.prop["binning"] = (
            f"{'Binning':<25s}|{str(self.binning)+'x'+str(self.binning):^9s}"
        )
        self.prop["dark_current"] = (
            f"{'Dark current [e-/px/s]':<25s}|{self.darkCurrent:^9.2f}"
        )
        self.prop["photon_noise"] = f"{'Photon noise':<25s}|{str(self.photonNoise):^9s}"
        self.prop["bkg_noise"] = (
            f"{'Bkg noise [e-]':<25s}|{str(self.backgroundNoise):^9s}"
        )
        self.prop["readout_noise"] = (
            f"{'Readout noise [e-/px]':<25s}|{self.readoutNoise:^9.1f}"
        )
        return self.prop
