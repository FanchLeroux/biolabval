...
# imports
import numpy as np
from biolabval.OOPAO.tools.interpolateGeometricalTransformation import interpolate_image
...
class FieldTransformer:
    ...
    def relay(self, src):
        ...
        for src in self.src_list:
            ...
            src.intensity = interpolate_image(
                image_in=src.fluxMap.copy(),
                pixel_size_in=1.0,
                pixel_size_out=1.0,
            ...
            # by updating the OPD, the phase (and OPD_no_pupil) will be also updated accordingly automatically
            src.OPD = interpolate_image(
                image_in=src.OPD.copy(),
                pixel_size_in=1.0,
                pixel_size_out=1.0,
                resolution_out=len(src.OPD.copy()),
                rotation_angle=self.rotation_angle[src.ast_idx],
    ...
    def properties(self) -> dict:
        ...
        self.prop["rotation_angle"] = (
            f"{'Rotation ang. [deg.]':<25s}|{self.rotation_angle}"
        )
        self.prop["anamorphosisAngle"] = (
            f"{'Anamorphosis ang. [deg.]':<25s}|{self.anamorphosisAngle}"
        )
        self.prop["tangentialScaling"] = (
            f"{'Tangential scaling':<25s}|{self.tangentialScaling}"
        )
