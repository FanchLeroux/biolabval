...
from biolabval.OOPAO.tools.tools import set_binning
...
class LiFT:
    def __init__(
        ...
        if (
            self.basis.shape[0] != self.tel.resolution
            and self.basis.shape[1] != self.tel.resolution
        ):
            raise ValueError(
                "The modal basis resolution (%i,%i) does not match the telescope resolution %i",
                self.basis.shape[0],
                self.basis.shape[1],
                self.tel.resolution,
            )
        ...
        if ang_pixel_arcsec is not None:
            ...
            self.zeroPaddingFactor = (self.tel.src.wavelength / self.tel.D) * (
                1 / (self.ang_pixel_rad)
            )
            print(
                "Using user-input PSF sampling:%.1f Pixels/FWHM "
            ...
            self.ang_pixel_rad = (
                self.tel.src.wavelength / self.tel.D
            ) / self.zeroPaddingFactor
            self.ang_pixel_arcsec = self.ang_pixel_rad * ((180 / np.pi) * 3600)
            print(
                "Using detector PSF sampling:%.1f Pixels/FWHM " % self.zeroPaddingFactor
            )
    ...
    def obj_convolve(self, mat):
        ...
        if self.object is not None:
            return (
                xg.convolve2d(mat, self.object, boundary="symm", mode="same")
                / self.object.sum()
            )
        else:
            return mat
    ...
    def generateLIFTinteractionMatrices(self, coefs, modes_ids, flux_norm=1.0):
        ...
        if not self.numerical:
            ...
            initial_amplitude = xp.sqrt(
                self.tel.pupilReflectivity
                * self.tel.src.nPhoton
                * flux_norm
                * self.tel.samplingTime
            ...
            for i in modes_ids:
                ...
                aux_oversampling = self.tel.PropagateField(
                    np.squeeze(self.basis[:, :, i]) * initial_amplitude,
                    initial_phase,
                    self.zeroPaddingFactor,
                    self.img_resolution,
                )
                ...
                derivative = (
                    2 * set_binning((xp.real(1j * buf * Pd)), aux_oversampling) * k
                )
            ...
            for i in modes_ids:
                ...
                self.tel.PropagateField(
                    xp.sqrt(self.tel.src.fluxMap),
                    k * self.tel.OPD,
                    self.zeroPaddingFactor,
                    self.img_resolution,
                )
    ...
    def Reconstruct(
        ...
        def criterion(i):
            return xp.abs(C[i] - C[i - 1]) / C[i]

        def coefs_norm(v):
            return xp.linalg.norm(v[modes], ord=2)

        def inverse_Rn(Rn):
            return 1.0 / xp.clip(Rn.flatten(), a_min=1e-6, a_max=Rn.max())
        ...
        for i in range(self.iterations):
            ...
            if i > 0 and (
                criterion(i) < 1e-6 or coefs_norm(A_ests[i] - A_ests[i - 1]) < 1e-12
            ):
                if verbous:
