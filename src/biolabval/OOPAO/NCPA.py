...
class NCPA:
    ...
    def NCPA_f2_law(self, f2):
        if type(f2) is list and len(f2) == 4:
            if self.basis == "KL":
                ...
                phase = np.sum(
                    [
                        np.random.RandomState(i * self.seed).randn()
                        / np.sqrt(i + f2[3])
                        * self.B[:, :, i]
                        for i in range(f2[1], f2[2])
                    ],
                    axis=0,
                )
            ...
            if self.basis == "Zernike":
                ...
                self.OPD = (
                    self.OPD / np.std(self.OPD[np.where(self.tel.pupil == 1)]) * f2[0]
                )
            ...
            if self.basis == "M2C":
                if self.M2C is not None:
                    ...
                    self.OPD = (
                        phase / np.std(phase[np.where(self.tel.pupil == 1)]) * f2[0]
                    )
                else:
                    raise TypeError(
    ...
    def KL_basis(self):
        from biolabval.OOPAO.calibration.compute_KL_modal_basis import compute_KL_basis
    ...
    def properties(self) -> dict:
        ...
        self.prop["amplitude"] = (
            f"{'Amplitude [nm RMS]':<20s}|{np.std(self.OPD[np.where(self.tel.pupil > 0)])*1e9:^9.1f}"
        )
        return self.prop
