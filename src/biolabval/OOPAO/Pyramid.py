...
class Pyramid:
    ...
    def mask_computation(self):
        ...
        if self.old_mask:
            self.m = self.get_phase_mask_old(
    ...
    def apply_shift_wfs(self, sx=None, sy=None, mis_reg=None, units="pixels"):
        ...
        if (
            np.max(np.abs(shift_x)) > self.n_pix_edge
            or np.max(np.abs(shift_y)) > self.n_pix_edge
        ):
            warning(
                "The Pyramid pupils have been shifted outside of the detector!"
    ...
    def get_phase_mask(
        ...
        rooftop_pixels = self.rooftop / np.sqrt(
            2
        )  # ♦ * self.zeroPaddingFactor / np.sqrt(2)
        # size of the mask in pixel
        n_tot = int(
            (n_subap * 2 + n_pix_separation + n_pix_edge * 2)
            * self.telescope.resolution
            / self.nSubap
        )
        ...
        x = np.linspace(
            -(1 - self.psfCentering * 1 / n_tot) * lim,
            (1 - self.psfCentering * 1 / n_tot) * lim,
            n_tot,
            endpoint=self.psfCentering,
        )
    ...
    def get_phase_mask_old(
        ...
        if psf_centering:
            ...
            m[: n_tot // 2, : n_tot // 2] = (
                Tip * (n_subap + n_pix_separation + sx[0])
                + Tilt * (n_subap + n_pix_separation - sy[0])
            ) * norma
            m[: n_tot // 2, -n_tot // 2 :] = (
                -Tip * (n_subap + n_pix_separation - sx[1])
                + Tilt * (n_subap + n_pix_separation - sy[1])
            ) * norma
            m[-n_tot // 2 :, -n_tot // 2 :] = (
                -Tip * (n_subap + n_pix_separation - sx[2])
                - Tilt * (n_subap + n_pix_separation + sy[2])
            ) * norma
            m[-n_tot // 2 :, : n_tot // 2] = (
                Tip * (n_subap + n_pix_separation + sx[3])
                - Tilt * (n_subap + n_pix_separation + sy[3])
            ) * norma
        else:
            ...
            [Tip_1, Tilt_1] = np.meshgrid(
                np.linspace(-lim_p, lim_p, n_tot // 2 + 1, endpoint=True),
                np.linspace(-lim_p, lim_p, n_tot // 2 + 1, endpoint=True),
            )
            [Tip_2, Tilt_2] = np.meshgrid(
                np.linspace(-lim_p, lim_p, n_tot // 2 + 1, endpoint=True),
                np.linspace(-lim_m, lim_m, n_tot // 2 - 1, endpoint=False),
            )
            ...
            m[: n_tot // 2 + 1, : n_tot // 2 + 1] = (
                Tip_1 * (n_subap + n_pix_separation + sx[0])
                + Tilt_1 * (n_subap + n_pix_separation - sy[0])
            ) * norma
            m[: n_tot // 2 + 1, -n_tot // 2 + 1 :] = (
                -Tip_4 * (n_subap + n_pix_separation - sx[1])
                + Tilt_4 * (n_subap + n_pix_separation - sy[1])
            ) * norma
            m[-n_tot // 2 + 1 :, -n_tot // 2 + 1 :] = (
                -Tip_3 * (n_subap + n_pix_separation - sx[2])
                - Tilt_3 * (n_subap + n_pix_separation + sy[2])
            ) * norma
            m[-n_tot // 2 + 1 :, : n_tot // 2 + 1] = (
                Tip_2 * (n_subap + n_pix_separation + sx[3])
                - Tilt_2 * (n_subap + n_pix_separation + sy[3])
            ) * norma
    ...
    def pyramid_transform(self, phase_in):
        ...
        if np.ndim(self.src.OPD) == 2 or type(self.src.OPD) is list:
            if self.modulation == 0:
                ...
                em_field = self.maskAmplitude * np.exp(
                    1j * (self.convert_for_gpu(self.src.phase) + phase_in)
                )
        else:
            em_field = self.maskAmplitude * np.exp(1j * phase_in)
        ...
        if self.psfCentering:
            ...
            em_field_pwfs = xp.fft.ifft2(em_field_ft * self.mask).astype(
        ...
        # case with mask centered on 1 pixel
            ...
            if self.spatialFilter is not None:
                ...
                em_field_ft = xp.fft.fftshift(fft2(support)).astype(
                    self.precision_complex()
                )
            em_field_pwfs = xp.fft.ifft2(em_field_ft * self.mask).astype(
                self.precision_complex()
        ...
        if self.modulation == 0 and self.user_modulation_path is None:
            if np.ndim(phase_in) == 2:
                self.raw_data = self.convert_for_numpy(
                    self.pyramid_transform(self.convert_for_gpu(self.src.phase))
                )
                if integrate:
                        ...
                        if i < nCycle - 1:
                            ...
                            def job_loop_single_mode_modulated():
                                Q = Parallel(
                                    ...
                                    delayed(self.pyramid_transform)(i)
    ...
    def grabQuadrant(self, n, cameraFrame=None):
        ...
        nExtraPix = int(
            np.round(
                (self.n_pix_separation / self.nSubap)
                * self.telescope.resolution
        ...
        if n == 3:
            quadrant = cameraFrame[
                nExtraPix + centerPixel : (nExtraPix + centerPixel + n_pixels),
                nExtraPix + centerPixel : (nExtraPix + centerPixel + n_pixels),
            ]
        if n == 4:
            quadrant = cameraFrame[
                nExtraPix + centerPixel : (nExtraPix + centerPixel + n_pixels),
                -nExtraPix + centerPixel - n_pixels : (-nExtraPix + centerPixel),
            ]
        if n == 1:
            quadrant = cameraFrame[
                -nExtraPix + centerPixel - n_pixels : (-nExtraPix + centerPixel),
