...
class ShackHartmann:
    def __init__(
        ...
        ************************** PROPERTIES **************************
        ...
        self.pixel_scale_init = (
            self.rad2arcsec
            * self.telescope.src.wavelength
            / self.d_subap
            / self.zero_padding
        )
        if pixel_scale is None:
            print(
                "No user-input pixel scale - using shannon_sampling input value:"
                + str((1 + self.shannon_sampling))
                + " pixel(s) per spot FWHM"
            )
            self.pixel_scale = self.pixel_scale_init * (2 - self.shannon_sampling)
            self.binning_pixel_scale = int(2 - self.shannon_sampling)
        else:
            print(
                "User input pixel scale - The shannon_sampling input value is ignored"
            )
            ...
            if binning_pixel_scale < 0.95:
                while binning_pixel_scale < 0.95:
                    ...
                    binning_pixel_scale = pixel_scale / self.pixel_scale_init
            binning_pixel_scale = [
                np.floor(pixel_scale / self.pixel_scale_init),
                np.ceil(pixel_scale / self.pixel_scale_init),
            ]
            ind_ = np.argmin(
                [
                    np.abs(
                        binning_pixel_scale[0] * self.pixel_scale_init - pixel_scale
                    ),
                    np.abs(
                        binning_pixel_scale[1] * self.pixel_scale_init - pixel_scale
                    ),
                ]
            )
            ...
            if pixel_scale != self.pixel_scale:
                warning(
                    "The requested pixel-scale is: "
                    + str(pixel_scale)
                    + " arcsec\n"
                    + "The effective pixel-scale is: "
                    + str(self.pixel_scale)
                    + " arcsec"
                )
        ...
        if n_pixel_per_subaperture is None:
            ...
            if (
                n_pixel_per_subaperture * self.pixel_scale
                > self.n_pix_subap_init * self.pixel_scale_init * self.zero_padding
                and self.is_LGS is False
            ):
                warning(
                    "The requested number of pixel per subaperture is too large!\n"
                    + "The SH spots will be zero-padded to provide the desired number but will not contain any signal.\n"
                    + "The FoV is limited to +/- "
                    + str(self.n_pix_subap_init * self.pixel_scale_init / 2)
                    + " arcsec. Any signal further will be wrapping.\n"
        ...
        self.outerMask = np.ones(
            [
                self.n_pix_subap_init * self.zero_padding,
                self.n_pix_subap_init * self.zero_padding,
            ]
        )
        ...
        self.phasor = np.exp(
            -(
                ...
                * (
                    ...
                    + 1
                    + (self.pixel_scale / self.pixel_scale_init) * self.half_pixel_shift
                )
                / self.n_pix_lenslet_init
            )
            * (xx + yy)
        ...
        for i_src, src in enumerate(self.src_list):
            ...
            self.set_valid_subaperture(
                src=src, sh_data=self.sh_data["src_" + str(i_src)]
            )
            # Set the pupil masks for each SH (needed for the geometric SH implementation with EM field transform)
            self.sh_data["src_" + str(i_src)].pupil_mask = src.intensity == np.max(
                src.intensity
            )
            ...
            if src.type == "LGS" and self.is_geometric is False:
                ...
                (
                    _,
                    _,
                    self.sh_data["src_" + str(i_src)].spot_kernel_elongation_fft,
                    self.sh_data["src_" + str(i_src)].spot_kernel_elongation,
    ...
    def initialize_flux(self, src, sh_data):
        ...
        for i in range(self.nSubap):
            ...
            sh_data.cube_flux[
                i * self.nSubap : (i + 1) * self.nSubap,
                self.center_init
                - self.n_pix_subap_init // 2 : self.center_init
                + self.n_pix_subap_init // 2,
                self.center_init
                - self.n_pix_subap_init // 2 : self.center_init
                + self.n_pix_subap_init // 2,
            ] = np.asarray(tmp_flux_v_split)
        # required to compute the valid subapertures
        sh_data.photon_per_subaperture = np.apply_over_axes(
            np.sum, sh_data.cube_flux, [1, 2]
        )
    ...
    def set_valid_subaperture(self, src, sh_data):
        self.photon_per_subaperture_2D = np.reshape(
            sh_data.photon_per_subaperture, [self.nSubap, self.nSubap]
        )
        sh_data.valid_subapertures = np.reshape(
            sh_data.photon_per_subaperture
            >= self.lightRatio * np.max(sh_data.photon_per_subaperture),
            [self.nSubap, self.nSubap],
        )
        sh_data.valid_subapertures_1D = np.reshape(
            sh_data.valid_subapertures, [self.nSubap**2]
        )
        [sh_data.validLenslets_x, sh_data.validLenslets_y] = np.where(
            self.photon_per_subaperture_2D
            >= self.lightRatio * np.max(sh_data.photon_per_subaperture)
        )
        # index of valid slopes X and Y
        sh_data.valid_signal_2D = np.concatenate(
            (sh_data.valid_subapertures, sh_data.valid_subapertures)
        )
    ...
    def relay(self, src):
        ...
        for i_src, src in enumerate(self.src_list):
            ...
            self.wfs_measure(
                phase_in=src.phase, src=src, sh_data=self.sh_data["src_" + str(i_src)]
            )
    ...
    def wfs_integrate(self, src, sh_data):
        ...
        self.centroid_lenslets = self.centroid(
            self.maps_intensity * sh_data.weighting_map, self.threshold_cog
        )
        ...
        # compute slopes-maps
        self.SX[sh_data.validLenslets_x, sh_data.validLenslets_y] = (
            self.centroid_lenslets[:, 0]
        )
        self.SY[sh_data.validLenslets_x, sh_data.validLenslets_y] = (
            self.centroid_lenslets[:, 1]
        )
    ...
    def wfs_measure(self, src, sh_data, phase_in=None, integrate=True):
        ...
        if self.is_geometric is False:
            if np.ndim(src.phase) == 2:
                ...
                if src.phase_filtered is None:
                    ...
                    self.initialize_flux(
                        ((src.amplitude_filtered) ** 2).T * src.intensity.T
                    )
                intensity = (
                    np.abs(
                        np.fft.fft2(
                            np.asarray(
                                self.get_lenslet_em_field(
                                    src=src, sh_data=sh_data, phase=phase
                                )
                            ),
                            axes=[1, 2],
                        )
                ...
                if self.is_LGS:
                    if self.convolution_tag == "FFT":
                        ...
                        extra_pixel = (
                            sh_data.spot_kernel_elongation_fft.shape[1]
                            - intensity.shape[1]
                        ) // 2
                        intensity = np.pad(
                            intensity,
                        ...
                        intensity = np.fft.fftshift(
                            np.abs(
                                (
                                    np.fft.ifft2(
                                        np.fft.fft2(intensity)
                                        * sh_data.spot_kernel_elongation_fft
                                    )
                                )
                            ),
                            axes=[1, 2],
                        ...
                        def joblib_convolve_direct():
                            Q = Parallel(n_jobs=12, prefer="threads")(
                                delayed(self.convolve_direct)(i, j)
                                for i, j in zip(
                                    sh_data.spot_kernel_elongation, intensity
                                )
                            )
                            return Q
                    ...
                    if self.pixel_scale == self.pixel_scale_init:
                        ...
                        raise OopaoError(
                            "The smallest pixel scale value is "
                            + str(self.pixel_scale_init)
                            + ' "'
                        )
                    else:
                        # pad the intensity to provide the right number of pixel before binning
                        self.extra_pixel = (
                            self.binning_pixel_scale * self.n_pix_subap_init
                            - intensity.shape[1]
                        ) // 2
                        intensity = np.pad(
                            intensity,
                ...
                if n_crop > 0:
                    ...
                    intensity = np.pad(
                        intensity, [[0, 0], [-n_crop, -n_crop], [-n_crop, -n_crop]]
                    )
                ...
                def joblib_fill_raw_data():
                    Q = Parallel(n_jobs=1, prefer="processes")(
                        delayed(self.fill_raw_data)(i, j, k)
                        for i, j, k in zip(
                            self.index_x[sh_data.valid_subapertures_1D],
                ...
                def compute_diffractive_signals_multi():
                    Q = Parallel(n_jobs=1, prefer="processes")(
                        delayed(self.wfs_measure)(phase_in=i, src=src, sh_data=sh_data)
                        for i in self.phase_buffer
                    )
                    return Q
                ...
                self.signal_2D = np.zeros(
                    [m[0][0].shape[0], m[0][0].shape[1], self.phase_buffer.shape[0]]
                )
            ...
            if np.ndim(src.phase) == 2:
                self.signal_2D = (
                    self.lenslet_propagation_geometric(src.phase, sh_data.pupil_mask)
                    * sh_data.valid_signal_2D
                    / self.slopes_units
                )
            ...
            # Geometric SH with multiple WFS
                ...
                def compute_geometric_signals():
                    Q = Parallel(n_jobs=1, prefer="processes")(
                        delayed(self.lenslet_propagation_geometric)(
                            arr=i, pupil_mask=sh_data.pupil_mask
                        )
                        for i in self.phase_buffer
                    )
                    return Q
    ...
    def set_weighted_centroiding_map(
        self, src, is_lgs: bool, is_gaussian: bool, fwhm_factor, sh_data=None
    ):
        """
        ...
        if src.tag == "asterism":
            for i_src, src_ast in enumerate(src.src):
                self.set_weighted_centroiding_map(
                    is_lgs=is_lgs,
            ...
            if is_lgs:
                ...
                _, _, _, weighting_map = self.get_convolution_spot(
                    fwhm_factor=fwhm_factor,
                    compute_fft_kernel=False,
                    is_gaussian=is_gaussian,
                    src=src,
                    sh_data=sh_data,
                )
                # bin the resulting image to the right pixel scale
                weighting_map = bin_ndarray(
                    weighting_map,
                    [
                ...
                print(
                    "The weighting map is a centerd gaussian 2D function with a FWHM of "
                    + str(fwhm_factor)
                    + " px"
                )
                weighting_map = np.tile(
                    gaussian_2D(resolution=self.n_pix_subap, fwhm=fwhm_factor),
                    [sh_data.nValidSubaperture, 1, 1],
                )
    ...
    def set_slopes_units(self, src=None, tomographic_reconstructor=None, dm=None):
        ...
        if tomographic_reconstructor is None:
            TT_in = OPD_map(
                ((self.Tip + self.Tilt) * src.wavelength / 2 / np.pi)
                * self.pixel_scale
                / (src.wavelength / self.telescope.D)
                / self.rad2arcsec
            )
            ...
            if src.type != "asterism":
                raise OopaoError(
                    "Only asterisms objects can be used to calibrate the SH WFS with a tomographic reconsctrutor"
                )
    ...
    def get_lenslet_em_field(self, src, sh_data, phase):
        ...
        for i in range(self.nSubap):
            ...
            complex_field = np.asarray(tmp_amp_v_split) * np.exp(
    ...
    def fill_raw_data(self, ind_x, ind_y, intensity, index_frame=None):
        if index_frame is None:
            self.raw_data[
                ind_x
                * self.n_pix_subap
                // self.binning_factor : (ind_x + 1)
                * self.n_pix_subap
                // self.binning_factor,
                ind_y
                * self.n_pix_subap
                // self.binning_factor : (ind_y + 1)
                * self.n_pix_subap
                // self.binning_factor,
            ] = intensity
        else:
    ...
    def split_raw_data(self, sh_data=None, input_frame=None):
        ...
        for i in range(self.nSubap):
            ...
            maps_intensity[
                i * self.nSubap : (i + 1) * self.nSubap,
                center
                - self.n_pix_subap // self.binning_factor // 2 : center
                + self.n_pix_subap // self.binning_factor // 2,
                center
                - self.n_pix_subap // self.binning_factor // 2 : center
                + self.n_pix_subap // self.binning_factor // 2,
            ] = np.asarray(raw_data_v_split)
    ...
    def compute_raw_data_multi(self, intensity, sh_data):
        ...
        index_x = np.tile(
            self.index_x[sh_data.valid_subapertures_1D], self.phase_buffer.shape[0]
        )
        index_y = np.tile(
            self.index_y[sh_data.valid_subapertures_1D], self.phase_buffer.shape[0]
        )
        for i in range(self.phase_buffer.shape[0]):
            self.ind_frame[
                i * sh_data.nValidSubaperture : (i + 1) * sh_data.nValidSubaperture
            ] = i

        def joblib_fill_raw_data():
            Q = Parallel(n_jobs=1, prefer="processes")(
                delayed(self.fill_raw_data)(i, j, k, l)
                for i, j, k, l in zip(index_x, index_y, intensity, self.ind_frame)
            )
            return Q
    ...
    def gradient_2D(self, arr, pupil_mask):
        ...
        res_x = (
            np.gradient(arr, axis=0, edge_order=1) / self.telescope.pixelSize
        ) * pupil_mask
        res_x = np.nan_to_num(res_x)
        res_y = (
            np.gradient(arr, axis=1, edge_order=1) / self.telescope.pixelSize
        ) * pupil_mask
    ...
    def lenslet_propagation_geometric(self, arr, pupil_mask):
        ...
        sy = bin_ndarray(
            ndarray=SLx,
            new_shape=(self.nSubap, self.nSubap),
            operation="mean",
            ignore_zeros=True,
        )
        sx = bin_ndarray(
    ...
    def get_convolution_spot(
        self, src, sh_data, fwhm_factor=1, compute_fft_kernel=False, is_gaussian=False
    ):
        ...
        x_subap = np.linspace(
            -self.telescope.D // 2, self.telescope.D // 2, self.nSubap
        )
        y_subap = np.linspace(
            -self.telescope.D // 2, self.telescope.D // 2, self.nSubap
        )
        ...
        for i in range(len(src.Na_profile[0, :])):
            coordinates_3D[:2, i] = (self.telescope.D / 4) * (
                [X0, Y0] / src.Na_profile[0, i]
            )
            coordinates_3D[2, i] = (
                self.telescope.D**2.0
                / (8.0 * src.Na_profile[0, i])
                / (2.0 * np.sqrt(3.0))
            )
            coordinates_3D_ref[:, i] = (
                coordinates_3D[:, i] - coordinates_3D[:, len(src.Na_profile[0, :]) // 2]
            )
        ...
        r_max = np.sqrt(
            (shift_X[0] - shift_X[-1]) ** 2 + (shift_Y[0] - shift_Y[-1]) ** 2
        )
        ...
        if n_even / self.binning_pixel_scale % 2 != 0:
            n_even = int(
                (np.ceil(n_even / self.binning_pixel_scale / 2))
                * 2
                * self.binning_pixel_scale
            )
            ...
            for j_subap in range(len(y_subap)):
                ...
                if sh_data.valid_subapertures_1D[count]:
                    ...
                    for i in range(len(src.Na_profile[0, :])):
                        ...
                        coordinates_3D_ref[:, i] = (
                            coordinates_3D[:, i]
                            - coordinates_3D[:, len(src.Na_profile[0, :]) // 2]
                        )
                        # shift in the focal planee (in rad) associated to the LGS
                        ...
                        if is_gaussian is False:
                            intensity += src.Na_profile[1, :][i] * gaussian_2D(
                                resolution=n_pix,
                                fwhm=sigma_spot,
                    ...
                    r = np.sqrt(
                        (
                            np.max(shift_X * self.pixel_scale_init)
                            - np.min(shift_X * self.pixel_scale_init)
                        )
                        ** 2
                        + (
                            np.max(shift_Y * self.pixel_scale_init)
                            - np.min(shift_Y * self.pixel_scale_init)
                        )
                        ** 2
                    )
                    theta = (
                        np.pi
                        + (np.pi / 2 + (np.arctan(-shift_X[-1] / shift_Y[-1])))
                        + np.pi / 2
                    )
                    if is_gaussian:
                        intensity = gaussian_2D(
                            resolution=n_pix,
                            fwhm=[
                                fwhm_factor * r / self.pixel_scale_init,
                                fwhm_factor
                                * self.src.fwhm_spot_up
                                / self.pixel_scale_init,
                            ],
                            position=[0, 0],
                            theta=theta,
                    ...
                    intensity[
                        intensity < self.threshold_convolution * intensity.max()
                    ] = 0
        ...
        return (
            np.asarray(shift_x_buffer),
            np.asarray(shift_y_buffer),
            np.asarray(spot_kernel_elongation_fft),
            np.asarray(spot_kernel_elongation),
        )
    ...
    def half_pixel_shift(self, val):
        ...
        if hasattr(self, "isInitialized"):
            if self.isInitialized:
                ...
                [xx, yy] = np.meshgrid(
                    np.linspace(
                        0, self.n_pix_lenslet_init - 1, self.n_pix_lenslet_init
                    ),
                    np.linspace(
                        0, self.n_pix_lenslet_init - 1, self.n_pix_lenslet_init
                    ),
                ...
                self.phasor = np.exp(
                    -(
                        ...
                        * (
                            self.n_pix_lenslet_init
                            + 1
                            + (self.pixel_scale / self.pixel_scale_init)
                            * self.half_pixel_shift
                        )
                        / self.n_pix_lenslet_init
                    )
                    * (xx + yy)
    ...
    def properties(self) -> dict:
        self.prop = dict()
        self.prop["subapertures"] = (
            f"{'Subapertures [lenslets]':<25s}|{self.nSubap:^9d}"
        )
        self.prop["subapertures_sky"] = (
            f"{'Subaperture Pitch [m]':<25s}|{self.telescope.D/self.nSubap:^9.2f}"
        )
        self.prop["fov"] = (
            f"{'Subaperture FoV [arcsec]':<25s}|{self.pixel_scale*self.n_pix_subap:^9.2f}"
        )
        ...
        self.prop["n_valid_pixels"] = (
            f"{'Valid Subapertures':<25s}|{self.nSignal:^9.0f}"
        )
        if self.is_LGS and self.is_geometric is False:
            for src in self.src_list:
                self.prop["spot_sampling"] = (
                    f"{'Spot Sampling [pix]':<25s}|{src.fwhm_spot_up/self.pixel_scale:^9.3f}"
                )
                self.prop["spot_elongation"] = (
                    f"{'Max Elongation [arcsec]':<25s}|{self.max_elongation_arcsec:^9.3f}"
                )
        else:
            self.prop["spot_sampling"] = (
                f"{'Spot Sampling [pix]':<25s}|{self.zero_padding*self.pixel_scale_init/self.pixel_scale:^9.2f}"
            )
