...
class Source:
    def __init__(
        ...
        ************************** EXEMPLE **************************
        ...
        # self.phase_no_pupil = []
        self._intensity = (
            None  # field intensity = fluxMap * scintillation (single array)
        )
    ...
    def scintillation_no_pupil(self, val):
        if val is not None:
            ...
            if len(val.shape) > 2 and np.isscalar(self.mask) is False:
                self.scintillation = (
                    self._scintillation_no_pupil * self.mask[:, :, np.newaxis]
                )
            else:
    ...
    def nPhoton(self, val):
        ...
        print(
            "Flux updated, magnitude is %2i and flux is %.2e"
            % (self._magnitude, self._nPhoton)
        )
        self.__updating_flux = False
    ...
    def get_theoretical_dsp(self, kappa, r0, L0, altitude):
        ...
        return compute_theoretical_psd(
            kappa=kappa, wavelength=self.wavelength, r0=r0, L0=L0, altitude=altitude
        )
    ...
    def properties(self) -> dict:
        ...
        self.prop["coordinates"] = (
            f"{'Coordinates [arcsec,deg]':<25s}|{' ['+str(self.coordinates[0])+','+str(self.coordinates[1])+']':s}"
        )
