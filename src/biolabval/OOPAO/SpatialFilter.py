...
class SpatialFilter:
    ...
    def set_spatial_filter(self, zeroPaddingFactor):
        ...
        if self.shape == "square":
            ...
            SF[
                self.center
                - self.diameter_padded // 2 : self.center
                + self.diameter_padded // 2,
                self.center
                - self.diameter_padded // 2 : self.center
                + self.diameter_padded // 2,
            ] = 1
        ...
        if valid_input is False:
            raise ValueError(
                "The input shape: '"
                + str(self.shape)
                + "' is not valid. The valid inputs are: 'circular', 'square', 'foucault'."
            )
    ...
    def relay(self, src):
        if src.tag == "source":
            ...
            src_list = src.src
        for src in src_list:
            em_field_in = xp.zeros(
                [self.resolution, self.resolution], dtype=self.precision_complex()
            )
            n_extra_pix = (self.resolution - src.OPD.shape[0]) // 2
            em_field_in[n_extra_pix:-n_extra_pix, n_extra_pix:-n_extra_pix] = np.sqrt(
                src.fluxMap
            ) * xp.exp(1j * (src.OPD_no_pupil * 2 * xp.pi / src.wavelength))
            em_field_focal_plane_filtered = xp.fft.fft2(em_field_in) * xp.fft.fftshift(
                self.mask
            )
            em_field_out = xp.fft.ifft2(em_field_focal_plane_filtered)
            src.em_field_filtered = em_field_out[
                n_extra_pix:-n_extra_pix, n_extra_pix:-n_extra_pix
            ]
