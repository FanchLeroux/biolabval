...
try:
    ...
    xp = np
from biolabval.OOPAO.tools.tools import set_binning, warning, OopaoError
...
class Telescope:
    ...
    def __init__(
        ...
        centralObstruction: float = 0.0,
        fov: float = 0.0,
        pupil: bool = None,
        pupilReflectivity: float = 1.0,
        ...
        ************************** EXEMPLE **************************
        ...
        self.static_OPD = np.zeros(
            [self.resolution, self.resolution], dtype=self.precision
        )
    ...
    def set_pupil(self):
        ...
        self.pupilReflectivity = (self.pupil * self.pupilReflectivity).astype(
            self.precision()
        )
        # Total number of pixels in the pupil area
        self.pixelArea = xp.sum(self.pupil)
        # index of valid pixels in the pupil
        self.pupilLogical = xp.where(
            xp.reshape(self.pupil, self.resolution * self.resolution) > 0
        )
        self.pupil = self.pupil

    def computeCoronoPSF(
        self, zeroPaddingFactor=2, display=False, coronagraphDiameter=4.5
    ):
        raise OopaoError(
            "The method computeCoronoPSF has been deprecated and is now integrated within the computePSF method setting the tel.coronograph_diameter property (default value is None and means no coronograph considered)"
        )
    ...
    def computePSF(self, zeroPaddingFactor=2, detector=None, img_resolution=None):
        ...
        if self.src is None:
            ...
            y_max = xp.abs(r * xp.sin(np.deg2rad(theta)))
        pixel_scale = (
            self.rad2arcsec * (input_source[0].wavelength / self.D) / zeroPaddingFactor
        )  # in arcsec
        maximum_fov = pixel_scale * img_resolution / 2
        n_extra = np.abs(
            np.floor((maximum_fov - max(x_max, y_max)) / pixel_scale)
            - img_resolution // 2
        )
        n_pix = np.ceil(
            max(int(img_resolution / 2 + n_extra) * 2, img_resolution)
        ).astype("int")
        ...
        for i_src in range(len(input_source)):
            ...
            x_shift = r * xp.sin(
                np.deg2rad(input_source[i_src].coordinates[1])
            )  # in arcsec
            y_shift = r * xp.cos(
                np.deg2rad(input_source[i_src].coordinates[1])
            )  # in arcsec
            ...
            delta_x = int(
                factor * np.floor(np.abs(x_shift) / pixel_scale) * np.sign(x_shift)
            )
            delta_y = int(
                factor * np.floor(np.abs(y_shift) / pixel_scale) * np.sign(y_shift)
            )
            ...
            self.delta_TT = (
                (delta_Tip * Tip + delta_Tilt * Tilt)
                * self.pupil
                * (self.D / input_source[i_src].wavelength)
                * (1 / self.rad2arcsec)
            )
            ...
            self.xPSF_arcsec = [
                -self.rad2arcsec
                * (input_source[i_src].wavelength / self.D)
                * (n_pix / 2 / zeroPaddingFactor),
                self.rad2arcsec
                * (input_source[i_src].wavelength / self.D)
                * (n_pix / 2 / zeroPaddingFactor),
            ]
    ...
    def PropagateField(self, amplitude, phase, zeroPaddingFactor, img_resolution=None):
        ...
        supportPadded = xp.pad(
            amplitude * xp.exp(1j * phase),
            pad_width=((pad_width, pad_width), (pad_width, pad_width)),
            constant_values=0,
        ).astype(self.precision_complex())
        ...
        if self.coronagraph_diameter is not None:
            [xx, yy] = xp.meshgrid(
                xp.linspace(0, N - 1, N, dtype=self.precision()),
                xp.linspace(0, N - 1, N, dtype=self.precision()),
            )
            xxc = xx - (N - 1) / 2
            yyc = yy - (N - 1) / 2
            self.apodiser = xp.sqrt(xxc**2 + yyc**2) < self.resolution / 2
            self.pupilSpiderPadded = xp.pad(
                self.pupil,
                pad_width=((pad_width, pad_width), (pad_width, pad_width)),
                constant_values=0,
            ).astype(self.precision_complex())
            self.focalMask = (
                xp.sqrt(xxc**2 + yyc**2)
                > self.coronagraph_diameter / 2 * zeroPaddingFactor
            )
            self.lyotStop = (
                xp.sqrt((xxc - 1.0) ** 2 + (yyc - 1.0) ** 2) < N / 2 * 0.9
            ) * self.pupilSpiderPadded
            # PSF computation
            [xx, yy] = xp.meshgrid(
                xp.linspace(0, N - 1, N, dtype=self.precision()),
                xp.linspace(0, N - 1, N, dtype=self.precision()),
            ...
            phasor = xp.exp(
                -1j * xp.pi / N * (xx + yy) * (1 - img_resolution % 2)
            ).astype(self.precision_complex)
            ...
            EMF = xp.fft.fftshift(
                1
                / N
                * xp.fft.fft2(xp.fft.ifftshift(supportPadded * phasor * self.apodiser))
            )
            self.B = EMF * self.focalMask * phasor
            self.C = (
                xp.fft.fftshift(1 * xp.fft.ifft2(xp.fft.ifftshift(self.B))).astype(
                    self.precision_complex
                )
                * self.lyotStop
                * phasor
            )
            EMF = (xp.fft.fftshift(1 * xp.fft.fft2(xp.fft.ifftshift(self.C)))).astype(
                self.precision_complex
            )
        else:
            # PSF computation
            ...
            self.phasor = xp.exp(
                -1j * xp.pi * (N + 1) / N * (xx + yy) * (1 - img_resolution % 2)
            ).astype(self.precision_complex())
            #                                                        ^--- this is to account odd/even number of pixels
            # Propagate with Fourier shifting
            EMF = xp.fft.fftshift(
                1 / N * xp.fft.fft2(xp.fft.ifftshift(supportPadded * self.phasor))
            ).astype(self.precision_complex())
            EMF = (1 / N * xp.fft.fft2((supportPadded * self.phasor))).astype(
                self.precision_complex()
            )
        ...
        ids = xp.array(
            [
                xp.ceil(N / 2) - img_size // 2 + (1 - N % 2) - 1,
                xp.ceil(N / 2) + img_size // 2 + shift_pix,
            ]
        ).astype(xp.int32)
    ...
    def apply_spiders(self, angle, thickness_spider, offset_X=None, offset_Y=None):
        ...
        if thickness_spider > 0:
            ...
            if (
                xp.max(xp.abs(offset_X)) >= max_offset
                or xp.max(xp.abs(offset_Y)) > max_offset
            ):
                warning("The spider offsets are too large! Weird things could happen!")
            for i in range(len(angle)):
                angle_val = (angle[i] + 90) % 360
                x = xp.linspace(
                    -self.D / 2, self.D / 2, self.resolution, dtype=self.precision()
                )
                [X, Y] = xp.meshgrid(x, x)
                X += offset_X[i]
                Y += offset_Y[i]
                map_dist = xp.abs(
                    X * xp.cos(xp.deg2rad(angle_val))
                    + Y * xp.sin(xp.deg2rad(-angle_val))
                )
                if 0 <= angle_val < 90:
    ...
    def pad(self, padding_values=0, sky_offset=None):
        ...
        if sky_offset is not None:
            if max(sky_offset) < n_extra_pix:
                pupil_padded[
                    n_extra_pix - sky_offset[0] : -n_extra_pix - sky_offset[0],
                    n_extra_pix - sky_offset[1] : -n_extra_pix - sky_offset[1],
                ] = self.initial_pupil
            else:
                raise OopaoError(
                    "The sky_offsets are too large for the considered pupil"
                )
        else:
            pupil_padded[n_extra_pix:-n_extra_pix, n_extra_pix:-n_extra_pix] = (
                self.initial_pupil
            )
    ...
    def OPD(self):
        if xp.ndim(self.src.OPD) == 2:
            self.mean_removed_OPD = (
                self.src.OPD - xp.mean(self.src.OPD[xp.where(self.pupil == 1)])
            ) * self.pupil
        return self.src.OPD
    ...
    def resetOPD(self):
        warning(
            "The use of resetOPD is deprecated. Consider using the Source reset option using the ** operator to reset the light propagation:\n src**tel"
        )
        self.src**self
        ...
        if type(obj) is list:
            ...
            if type(self.OPD) is list:
                if len(self.OPD) == len(obj):
                    ...
                    raise OopaoError(
                        "Error! There is a mis-match between the number of Sources ("
                        + str(len(self.OPD))
                        + ") and the number of WFS ("
                        + str(len(obj))
                        + ")"
                    )
            else:
    ...
    def properties(self) -> dict:
        ...
        self.prop["surface"] = (
            f"{'Surface [m²]':<25s}|{self.pixelArea*self.pixelSize**2:^10.2f}"
        )
        self.prop["obstruction"] = (
            f"{'Central obstruction [%]':<25s}|{self.centralObstruction*100:^10.0f}"
        )
        ...
        if self.src:
            if self.src.type == "asterism":
                for i_src in range(len(self.src.src)):
                    self.prop["source_%d" % i_src] = (
                        f"{'Source %s [m]'%self.src.src[i_src].type:<25s}|{self.src.src[i_src].wavelength:^10.2e}"
                    )
            else:
                self.prop["source_%d"] = (
                    f"{'Source %s [m]'%self.src.type:<25s}|{self.src.wavelength:^10.2e}"
                )
        return self.prop
