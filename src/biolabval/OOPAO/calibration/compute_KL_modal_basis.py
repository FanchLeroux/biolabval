...
def compute_M2C(
    ...
    if IF_2D is None:
        ...
        IF_2D = deformableMirror.modes.reshape(
            telescope.resolution, telescope.resolution, deformableMirror.nValidAct
        ) * np.tile(telescope.pupil[:, :, None], deformableMirror.nValidAct)
    ...
    if display:
        ...
        print(" ")
    GEO = aou.mkp(
        telescope.resolution / telescope.resolution * diameter,
    ...
    if nZer is not None and SpM_2D is None:
        ...
        from biolabval.OOPAO.Zernike import Zernike
    ...
    if SZ is None:
        ...
        )  ## SZ=1110 for dxo=0.06944 and SZ=1542 for dxo=0.05
    if display:
        print("COMPUTING VON KARMAN 2D PSD...")
        print(" ")
    PSD_atm, df, pterm = aou.VK_DSP_up(
        diameter, r0, L0, SZ, telescope.resolution, 1, pupil
    )
    ...
    if recompute_cov is True:
        ...
        if mem_available is None:
            mem_available = 100.0e9
        if NDIVL is None:
            mem, NDIVL = aou.estimate_ndivl(
                SZ, telescope.resolution, nact, mem_available
            )
        if NDIVL == 0:
            NDIVL = 1
        BLOCKL = nact // NDIVL
        ...
        if save_output:
            try:
                aou.save(
                    nameFolder + "HHt_PSD_df_" + HHtName + ".pkl", [HHt, PSD_atm, df]
                )
            except:
                aou.save(
                    nameFolder
    ...
    if minimF == True:
        if P2F is None:
            try:
                P2F = (
                    np.float64(pfits.getdata(param["pathInput"] + "P2F.fits")) * 1.0e6
                )  # ( in N/m)
                ...
                if nact > 892:
                    nap = nact // 6
                    for k in range(0, 6):
                        P2Ff[k * nap : (k + 1) * nap, k * nap : (k + 1) * nap] = (
                            P2F.copy()
                        )
    ...
    if computeSpM == True and minimF == True:
        ...
        SpM = aou.build_SpecificBasis_F(
            Tspm, IFma, DELTA, K, alpha, ortho_spm, check, amp_check
        )
        #        SpM_opd = IFma @ SpM
        if display:
        ...
        if display:
            print(
                "Orthonormality error for SpM = ",
                np.max(np.abs(DELTA_SpM_opd / tpup - np.eye(nspm))),
            )
    ...
    if computeSpM == True and minimF == False:
        ...
        SpM = aou.build_SpecificBasis_C(
            Tspm, IFma, DELTA, lim, ortho_spm, check, amp_check
        )
        if display:
            print("CHECKING ORTHONORMALITY OF SPECIFIC MODES...")
            print(" ")
    ...
    if computeSB == True:
        ...
        if minimF == False:
            ...
            if display:
                print(
                    "Orthonormality error for "
                    + str(nSB)
                    + " modes of the Seed Basis = ",
    ...
    if computeKL == True:
        ...
        if display:
            print(
                "Orthonormality error for " + str(nmoKL) + " modes of the KL Basis = ",
