...
def run_cl_first_stage(
    param,
    obj,
    speed_factor=2,
    filename_phase_screen=None,
    extra_name="",
    ...
    residual_phase_screen = np.zeros(
        [nLoop, obj.tel.resolution, obj.tel.resolution], dtype=np.float32
    )
    ...
    if destination_folder is None:
        destination_folder = (
            param["pathOutput"] + "/" + param["name"] + "/" + obj.wfs.tag + "/"
        )
    ...
    if obj.display:
        ...
        plot_obj = cl_plot(
            list_fig=[obj.tel_fast.OPD, obj.tel_fast.OPD],
    ...
    for i_loop in range(nLoop):
        ...
        # save phase variance
        ao_turbulence[i_loop] = (
            np.std(obj.atm_fast.OPD[np.where(obj.tel.pupil > 0)]) * 1e9
        )
        ...
        ao_residuals[i_loop] = (
            np.std(obj.tel_fast.OPD[np.where(obj.tel.pupil > 0)]) * 1e9
        )

        b = time.time()

        mean_rem_OPD = obj.tel_fast.OPD.copy()
        mean_rem_OPD[np.where(obj.tel.pupil > 0)] -= np.mean(
            mean_rem_OPD[np.where(obj.tel.pupil > 0)]
        )
        ...
        print(
            ...
            + str(nLoop)
            + "Turbulence: "
            + str(ao_turbulence[i_loop])
            + " -- Residual:"
            + str(ao_residuals[i_loop])
            + "\n"
        )
    ...
    if filename_phase_screen is None:
        filename_phase_screen = (
            "seeing_"
            + str(np.round(obj.atm.seeingArcsec, 2))
            + "_magnitude_"
            + str(np.round(obj.ngs.magnitude))
            + "_nKL_"
            + str(param["nModes"])
            + "_nLoop_"
            + str(param["nLoop"])
            + "_windspeed_"
            + str(np.round(np.mean(obj.atm_fast.windSpeed)))
            + "_stg1_"
            + str(np.round(0.001 / obj.tel.samplingTime, 2))
            + "_kHz_stg2_"
            + str(np.round(0.001 / obj.tel_fast.samplingTime, 2))
            + "_kHz"
        )
