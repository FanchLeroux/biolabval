...
def run_cl_from_phase_screens(param, ao_object, phase_screens):
    ...
    ao_object.ngs.nPhoton = (
        param["nPhotonPerSubaperture"]
        * (1 / ao_object.tel.samplingTime)
        / ((ao_object.tel.D / param["nSubaperture"]) ** 2)
    )
    ...
    if ao_object.display:
        ...
        plot_obj = cl_plot(
            list_fig=[phase_screens[0], phase_screens[0]],
    ...
    for i_loop in range(nLoop):
        ...
        ao_turbulence[i_loop] = (
            np.std(ao_object.tel.OPD[np.where(ao_object.tel.pupil > 0)]) * 1e9
        )
