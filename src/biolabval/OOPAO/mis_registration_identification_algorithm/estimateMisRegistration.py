...
from biolabval.OOPAO.tools.displayTools import cl_plot
from ..MisRegistration import MisRegistration
from ..calibration.InteractionMatrix import (
    InteractionMatrix,
    InteractionMatrixFromPhaseScreen,
)
from ..mis_registration_identification_algorithm.applyMisRegistration import (
    applyMisRegistration,
)
from ..mis_registration_identification_algorithm.computeMetaSensitivyMatrix import (
    computeMetaSensitivityMatrix,
)
from ..tools.interpolateGeometricalTransformation import (
...
def estimateMisRegistration(
    ...
    if sensitivity_matrices is None:
        [metaMatrix, calib_0] = computeMetaSensitivityMatrix(
    ...
    epsilonMisRegistration_field = list(
        np.asarray(epsilonMisRegistration_field)[ind_mis_reg]
    )
    stroke = 1e-9
    criteria = 0
    ...
    for i in range(n_mis_reg):
        misRegEstBuffer_ref[i] = getattr(
            misRegistrationZeroPoint, epsilonMisRegistration_field[i]
        )
    ...
    if plot:
        ...
        for i_mis_reg in range(n_mis_reg):
            list_inp.append(
                [
                    [0, misRegEstBuffer_ref[i_mis_reg]],
                    [0, misRegEstBuffer_ref[i_mis_reg]],
                ]
            )
        ...
        plot_obj = cl_plot(
            ...
            list_ratio=[[0.95, 0.1], [1] * (n_mis_reg)],
            s=20,
        )
    ...
    if fast:
        ...
        while criteria == 0:
            ...
            if np.ndim(input_modes_0) == 2:
                if wfs_mis_registrated is not None:
                    ...
                    wfs.apply_shift_wfs(
                        misRegistration_wfs.shiftX, misRegistration_wfs.shiftY
                    )

                    input_modes_cp = tel.pupil * apply_mis_reg(
                        tel, input_modes_0, misRegistration_dm
                    )
                else:
                    input_modes_cp = tel.pupil * apply_mis_reg(
                        tel, input_modes_0, misRegistration_out
                    )
            else:
                ...
                for i_modes in range(input_modes_0.shape[2]):
                    if wfs_mis_registrated is not None:
                        ...
                        misRegistration_dm = MisRegistration()
                        misRegistration_dm.rotationAngle = (
                            misRegistration_out.rotationAngle
                        )
                        ...
                        input_modes_cp[:, :, i_modes] = tel.pupil * apply_mis_reg(
                            tel, input_modes_0[:, :, i_modes], misRegistration_dm
                        )
                    else:
                        input_modes_cp[:, :, i_modes] = tel.pupil * apply_mis_reg(
                            tel, input_modes_0[:, :, i_modes], misRegistration_out
                        )

            # temporary interaction matrix
            calib_tmp = InteractionMatrixFromPhaseScreen(
                ngs,
                tel,
                wfs,
                input_modes_cp,
            ...
            try:
                scalingFactor_tmp = np.round(
                    np.diag(calib_tmp.D.T @ calib_in.D)
                    / np.diag(calib_tmp.D.T @ calib_tmp.D),
                    precision,
                )
                contain_nan = False
                ...
                misReg_tmp = gainEstimation * np.matmul(
                    metaMatrix.M,
                    np.reshape(
                        calib_in.D @ np.diag(1 / scalingFactor_tmp) - calib_tmp.D,
                        calib_in.D.shape[0] * calib_in.D.shape[1],
                    ),
                )
            except:
                scalingFactor_tmp = np.round(
                    np.sum(np.squeeze(calib_tmp.D) * np.squeeze(calib_in.D))
                    / np.sum(np.squeeze(calib_tmp.D) * np.squeeze(calib_tmp.D)),
                    precision,
                )
                contain_nan = False
                ...
                misReg_tmp = gainEstimation * np.matmul(
                    metaMatrix.M,
                    np.squeeze(
                        (np.squeeze(calib_in.D) * (1 / scalingFactor_tmp))
                        - np.squeeze(calib_tmp.D)
                    ),
                )
            ...
            for i_mis_reg in range(n_mis_reg):
                setattr(
                    misRegistration_out,
                    epsilonMisRegistration_field[i_mis_reg],
                    getattr(
                        misRegistration_out, epsilonMisRegistration_field[i_mis_reg]
                    )
                    + np.round(misReg_tmp[i_mis_reg], precision),
                )

            # save the data for each iteration
            scalingFactor_values.append(np.copy(scalingFactor_tmp))
            misRegistration_values.append(
                np.copy(misRegEstBuffer) + misRegEstBuffer_ref
            )
            if display:
                print(
                    "----------------------------------------------------------------------------"
            ...
            if plot:
                ...
                for i_mis_reg in range(n_mis_reg):
                    ...
                    list_inp.append(
                        [np.arange(len(misRegistration_values) - 1), global_value]
                    )
                    list_lim.append(
                        [
                            0.8 * misRegEstBuffer_ref[i_mis_reg],
                            1.2 * misRegEstBuffer_ref[i_mis_reg],
                        ]
                    )

                plot_obj.list_lim = list_lim
                cl_plot(list_fig=list_inp, plt_obj=plot_obj)
                plt.pause(0.01)
        ...
        while criteria == 0:
            ...
            if i_iter > 1:
                # temporary deformable mirror
                dm_tmp = applyMisRegistration(
                    tel,
                    misRegistration_out,
                    param,
                    wfs=wfs_mis_registrated,
                    print_dm_properties=False,
                    floating_precision=dm_0.floating_precision,
                    dm_input=dm_input,
                )

                # temporary interaction matrix
                calib_tmp = InteractionMatrix(
                    ngs,
                    tel,
                    dm_tmp,
                    wfs,
                    np.squeeze(basis.modes),
                    stroke,
                    phaseOffset=0,
                    nMeasurements=1,
                    invert=False,
    ...
    misRegistration_out.rotationAngle = np.round(
        misRegistration_out.rotationAngle, precision
    )
    misRegistration_out.radialScaling = np.round(
        misRegistration_out.radialScaling, precision
    )
    misRegistration_out.tangentialScaling = np.round(
        misRegistration_out.tangentialScaling, precision
    )
    ...
    if nIteration == 1:
        ...
        # values for validity
        tolerance = [
            np.rad2deg(np.arctan((dm_0.pitch * tolerance) / (tel.D / 2))),
            dm_0.pitch * tolerance,
            dm_0.pitch * tolerance,
            0.05,
            0.05,
        ]
        ...
        if np.argwhere(diff - tolerance[:n_mis_reg] > 0).size == 0 or contain_nan:
            ...
            validity_flag = False
            misRegistration_out.shiftX = 0 * np.round(
                misRegistration_out.shiftX, precision
            )
            misRegistration_out.shiftY = 0 * np.round(
                misRegistration_out.shiftY, precision
            )
            misRegistration_out.rotationAngle = 0 * np.round(
                misRegistration_out.rotationAngle, precision
            )
            misRegistration_out.radialScaling = 0 * np.round(
                misRegistration_out.radialScaling, precision
            )
            misRegistration_out.tangentialScaling = 0 * np.round(
                misRegistration_out.tangentialScaling, precision
            )
...
def apply_mis_reg(tel, map_2d, misReg):
    ...
    anamMatrix = anamorphosisImageMatrix(
        tel.OPD,
        misReg.anamorphosisAngle,
    ...
    shiftMatrix = translationImageMatrix(
        tel.OPD, [misReg.shiftY / pixelsize, misReg.shiftX / pixelsize]
    )  # units are in m
