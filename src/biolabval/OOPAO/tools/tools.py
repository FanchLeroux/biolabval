...
from biolabval.OOPAO.tools import *
...
def zero_pad_array(array, padding: int):
    ...
    if np.ndim(array) != 2:
        ...
        support = np.zeros(
            [int(2 * padding + array.shape[0]), int(2 * padding + array.shape[1])],
            dtype=array.dtype,
        )
        center_x = support.shape[0] // 2
        center_y = support.shape[1] // 2
        support[
            center_x - array.shape[0] // 2 : center_x + array.shape[0] // 2,
            center_y - array.shape[1] // 2 : center_y + array.shape[1] // 2,
        ] = array.copy()
        return support
...
def strehlMeter(PSF, tel, PSF_ref=None, zeroPaddingFactor=2, display=True, title=""):
    ...
    if display:
        ...
        plt.title(
            title + " Strehl " + str(np.round(np.sum(OTF) / np.sum(OTFa) * 100, 1))
        )
        plt.legend()
        plt.ylim((1e-4, 1))
        plt.xlim((0, 1))
...
def bin_ndarray(ndarray, new_shape, operation="sum", ignore_zeros=False):
    ...
    if ignore_zeros and operation == "mean":
        ...
        count_nonzero = np.count_nonzero(
            ndarray, axis=tuple(range(1, len(new_shape) * 2, 2))
        )
        with np.errstate(
        ...
        binned_array = op(
            axis=tuple(range(1, len(new_shape) * 2, 2))
        )  # apply method to obtain final binned array
...
def compute_fourier_mode(pupil, spatial_frequency, angle_deg, zeropadding=2):
    ...
    Z[
        N // 2 + int(t * np.cos(np.deg2rad(thet) + np.pi)),
        N // 2 + int(t * np.sin(np.deg2rad(thet) + np.pi)),
    ] = 1
    Z[
        N // 2 - int(t * np.cos(np.deg2rad(thet) + np.pi)),
        N // 2 - int(t * np.sin(np.deg2rad(thet) + np.pi)),
...
def set_binning(array, binning_factor, mode="sum"):
    if array.shape[0] % binning_factor == 0:
        if array.ndim == 2:
            new_shape = [
                int(np.round(array.shape[0] / binning_factor)),
        ...
        raise ValueError(
            "Binning factor %d not compatible with the array size" % (binning_factor)
        )
        ...
        if isinstance(position, list):
            if len(position) == 2:
                ...
                raise AttributeError(
                    "the input position should be a list of two elements"
                )
        else:
...
def compute_theoretical_pupil_log_amp_variance(
    wavelength, D_tel, L_source, L0, Cn2_input
):
    """
...
def compute_fresnel_padding(
    D_tel, resolution, wavelengths, z_max, r0_wvl_list, max_layer_step, res_factor=1
):
    """
