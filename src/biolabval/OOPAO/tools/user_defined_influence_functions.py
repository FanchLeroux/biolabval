...
from biolabval.OOPAO.calibration.ao_cockpit_psim import mkp
...
from biolabval.OOPAO.tools.interpolateGeometricalTransformation import (
    anamorphosisImageMatrix,
    interpolate_cube,
    rotateImageMatrix,
    translationImageMatrix,
)
from biolabval.OOPAO.tools.tools import OopaoError


# ---------------------------------------------------------------------------
# ELT - M4
# ---------------------------------------------------------------------------
def compute_ELT_M4_influence_functions(
    name_system: str,
    diameter: float,
    resolution: int,
    loc: str = None,
    ...
    IFMA = np.fromfile(
        loc + "SAVED_DATA/IFMA_FEM_123903x892.raw", dtype=np.float64
    ).reshape(n_nodes, nact_per_seg)
    if new_arrangement:
        IFMA = IFMA[:, idx]

    # -------------------- coordinate system in M1 --------------------
    diam_max = (resolution - 1) * pixel_size  # largest diameter in array
    cobs = 0.0  # central obstruction, not used
    ...
    XXp = XXp * (
        m_rad * np.cos(ana_angle) ** 2 + m_tan * np.sin(ana_angle) ** 2
    ) + YYp * (m_tan * np.sin(2 * ana_angle) / 2 - m_rad * np.sin(2 * ana_angle) / 2)
    YYp = YYp * (
        m_rad * np.sin(ana_angle) ** 2 + m_tan * np.cos(ana_angle) ** 2
    ) + XXp * (m_tan * np.sin(2 * ana_angle) / 2 - m_rad * np.sin(2 * ana_angle) / 2)
    ...
    def build_interp_matrix(points, query):
        ...
        w = np.column_stack([b, 1.0 - b.sum(axis=1)])  # [n_q, 3]
        w[~inside] = 0.0

        verts = tri.simplices[s]  # [n_q, 3]
        rows = np.repeat(np.arange(len(query)), 3)
        return csr_matrix(
            (w.ravel(), (rows, verts.ravel())), shape=(len(query), points.shape[0])
        )
...
# RAMA - DM97
# ---------------------------------------------------------------------------
def compute_RAMA_DM97_influence_functions(
    name_system: str,
    diameter: float,
    resolution: int,
    loc: str = None,
...
# LBT - ASM
# ---------------------------------------------------------------------------
def compute_LBT_ASM_influence_functions(
    name_system: str,
    diameter: float,
    resolution: int,
    loc: str = None,
    ...
    shift_matrix = translationImageMatrix(
        influ_map,
        [-mis_registration.shiftX / pixel_size, -mis_registration.shiftY / pixel_size],
    )  # units are in [m]
    ...
    transformation_matrix = (
        down_scaling + anam_matrix + rot_matrix + shift_matrix + alignment_matrix
    )
...
# GHOST - DM492
# ---------------------------------------------------------------------------
def compute_GHOST_DM492_influence_functions(
    name_system: str,
    diameter: float,
    resolution: int,
    loc: str = None,
...
# PAPYRUS - DM241
# ---------------------------------------------------------------------------
def compute_PAPYRUS_DM241_influence_functions(
    name_system: str,
    diameter: float,
    resolution: int,
    loc: str = None,
    ...
    DM_coordinates = DM_coordinates[
        dist <= D_T152 / 2 + 2.2 * pitch * D_T152 / T152_on_DM_size, :
    ]
...
# EKARUS - DM468
# ---------------------------------------------------------------------------
def compute_EKARUS_DM468_influence_functions(
    name_system: str,
    diameter: float,
    resolution: int,
    loc: str = None,
